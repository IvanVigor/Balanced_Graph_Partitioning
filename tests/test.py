...
class GraphPartitioning(object):
    ...
    def bilancedPartion(self, subTree, threshold, G):
        ...
        if subTree.data.number_of_nodes() <= 1:  # if subtree is a leaf, return it
            ...
            balanceCut = int(
                round(
                    ((self.epsilon / 3) / (1 + self.epsilon / 3))
                    / 2
                    * subTree.data.number_of_nodes()
                )
            )  # Evaluation of epsilon according to formula provided in paper
            listOfNodes = (
                subTree.data.nodes()
            )  # Selection of two randon nodes in order to divide the graph in two separate parts
    ...
    def getGVector(self):
        ...
        val = math.pow(
            (1 + self.epsilon / 2),
            math.floor(
                math.log(self.epsilon * self.n / (3 * self.k), (1 + self.epsilon / 2))
            ),
        )
    ...
    def getBestPartitionMethod1(
        ...
        for p in listPartitions:
            ...
            partition, costCurrentPartition = self.createPartitionNodes(
                listTrees,
                p,
                PossiblePartitionsCost,
                GVector,
            ...
            if len(packer.items) >= self.k and packer.pack_items(
                self.k
            ):  # Return True se riesce a farceli stare
                return partition, p
    ...
    def getBestPartitionMethod2(
        ...
        if index == len(listTrees):
            ...
            if vectorG in self.feasibleSets:
                ...
                vectorG not in self.unfeasibleSets
                and len(packer.items) >= self.k
                and packer.pack_items(self.k)
            ):  # Return True se riesce a farceli stare
    ...
    def evaluatePartitionsMethod1(self, listTrees, listTotalCosts):
        fin, PossiblePartitionsCost = self.orderFinalPartitions(
            listTotalCosts
        )  # get all partition ordered by cost
    ...
    for val in valuesOfK:
        ...
        for sample in range(samplesForValue):
            if nx.is_connected(G):
                ...
                partitor.bilancedPartion(
                    root, epsilon * G.number_of_nodes() / (3 * val), G
                )
    ...
    for i in range(len(gList)):
        ...
        for sample in range(samplesForValue):
            if nx.is_connected(gList[i]):
                ...
                # partitor.division(root,epsilon*n/(3*k),G)   # create Tree
                partitor.bilancedPartion(
                    root, epsilon * gList[i].number_of_nodes() / (3 * k), gList[i]
                )
                listTrees = partitor.pruneTree(root)
                listTotalCosts = partitor.scanTrees(listTrees)
                # bestP = partitor.evaluatePartitionsMethod1(listTrees, listTotalCosts)
